/**
 * Creates a Chrome browser interface with various browser API utilities
...
export default function ChromeBrowserInterface(browserApi) {
	...
	self.requestPermissions = async function (permissionsArray) {
		...
		console.log('[BrowserInterface Debug] Requesting permissions:', permissionsArray);
		try {
			// Check current permissions and request new ones
			const currentPerms = await chrome.permissions.getAll(),
				result = await chrome.permissions.request({ permissions: permissionsArray }),
				finalPerms = await chrome.permissions.getAll();
			console.log('[BrowserInterface Debug] Permission state:', {
				before: currentPerms,
			...
			if (!result) {
				console.error('[BrowserInterface Debug] Permission request denied by user');
				throw new Error('Permission request failed');
			}
			return true;
		} catch (error) {
			console.error('[BrowserInterface Debug] Permission request failed:', error);
	 ...
	 */
	self.removePermissions = async function (permissionsArray) {
		console.log('[BrowserInterface Debug] Removing permissions:', permissionsArray);
		try {
			const currentPerms = await chrome.permissions.getAll(),
				result = await new Promise((resolve) => {
					chrome.permissions.remove({ permissions: permissionsArray }, resolve);
				}),
				remainingPerms = await chrome.permissions.getAll();

			console.log('[BrowserInterface Debug] Permission removal state:', {
				before: currentPerms,
				removalResult: result,
				remaining: remainingPerms
			});
			return result;
		} catch (error) {
			console.error('[BrowserInterface Debug] Failed to remove permissions:', error);
			throw new Error(`Failed to remove permissions: ${error.message}`);
		}
	};

	// Enhanced permissions check with detailed logging
	self.checkPermissions = async function () {
		try {
			const perms = await chrome.permissions.getAll(),
				contextMenusEnabled = await chrome.permissions.contains({
					permissions: ['contextMenus']
				});
			console.log('[BrowserInterface Debug] Permission check:', {
				allPermissions: perms,
				hasContextMenus: contextMenusEnabled,
				hasScripting: !!chrome.scripting,
				hasStorage: !!chrome.storage
			});
			return perms;
		} catch (error) {
			console.error('[BrowserInterface Debug] Failed to check permissions:', error);
			return null;
		}
	};
