...
import ChromeConfigInterface from '../lib/chrome-browser-interface.mjs';
import initConfigWidget from '../lib/init-config-widget.mjs';

const addLink = (element, url) => {
	...
	saveOptions = () => {
		const additionalMenus = []; // This would be populated from the UI
	...
	rebuildMenu = () => {
		...
		const list = document.getElementById('configOptions'),
			...
			additionalMenus = []; // Example data

		console.log('Clearing menu list...');
		list.innerHTML = '';

		if (!template) {
			console.error('Error: Menu template not found');
			return;
		}
...
// Initialize configuration widget on DOMContentLoaded
document.addEventListener('DOMContentLoaded', () => {
	...
	try {
		...
		rebuildMenu(); // Initial menu build
	} catch (error) {
